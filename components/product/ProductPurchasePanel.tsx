  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="flex items-center justify-between border border-rule bg-paper-raised sm:justify-start">
          <button
            type="button"
            onClick={() => setQuantity((q) => Math.max(1, q - 1))}
            disabled={quantity <= 1}
            aria-label="Minska antal"
            className="px-4 py-2.5 text-lg text-ink transition hover:text-ink-muted disabled:opacity-30"
          >
            −
          </button>
          <span className="mono min-w-[2rem] text-center font-medium">
            {quantity}
          </span>
          <button
            type="button"
        ...
        <button
          ...
          onClick={handleAdd}
          disabled={!canAdd}
          className="flex-1 bg-ink px-6 py-3.5 text-center font-mono text-sm uppercase tracking-meta text-paper transition hover:bg-ink-muted disabled:cursor-not-allowed disabled:bg-ink-faint/40 disabled:text-ink-faint"
        >
          {!canAdd ? "Slut" : added ? "Tillagd" : "Lägg i varukorg"}
        </button>
      </div>
